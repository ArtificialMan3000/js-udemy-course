...
function calc() {
  ...
  const calculatorElem = document.querySelector('.calculating__field'); // Элемент с выбором пола

  const genderElem = calculatorElem.querySelector('#gender'); // Элемент с выбором конституции

  const constitutionElem = calculatorElem.querySelector('#constitution'); // Элемент с выбором физ. активности

  const activityElem = calculatorElem.querySelector('#activity'); // Элемент, содержащий число - результат вычислений

  const calculatingResultNumElem = calculatorElem.querySelector('.calculating__result > span'); // Коэффициенты активности
  ...
  const updateCalories = () => {
    // Получаем данные
    const data = getCalcData(); // Рассчитываем калории

    const calories = calcCalories(data.gender, data.weight, data.height, data.age, data.activity); // Выводим результат на страницу
  ...
  const switchButton = id => {
    // Целевая кнопка
    const targetButton = calculatorElem.querySelector(`#${id}`); // Группа кнопок, в которой происходит переключение

    const targetButtonGroup = targetButton.closest('.calculating__choose'); // Деактивируем все кнопки в группе

    deactivateButtonsGroup(targetButtonGroup); // Активируем целевую кнопку

    activateButton(targetButton); // Сохраняем данные в localStorage
...
function cards() {
  ...
  class MenuCard {
    constructor(imgSrc, imgAlt, title, description, price, ...classes) {
      ...
      this.changeToUAH(); // Создаём карточку

      this.createCardElem(); // Наполняем карточку данными
...
function forms() {
  ...
  const showThanksModal = message => {
    ...
    Object(_modal__WEBPACK_IMPORTED_MODULE_0__["openModal"])('.modal'); // Через 4 секунды после показа удаляем элемент с сообщением, возвращаем диалоговую часть и закрываем окно
  ...
  const bindPostData = form => {
    form.addEventListener('submit', evt => {
      ...
      Object(_services_services__WEBPACK_IMPORTED_MODULE_1__["postData"])('http://localhost:3000/requests', formDataJson).then(data => {
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  // Элемент модального окна
  const modalElem = document.querySelector(modalSelector); // Элемент диалоговой части модального окна

  const modalDialogElem = modalElem.querySelector('.modal__dialog'); // Кнопки вызова модального окна

  const modalCallElems = document.querySelectorAll(triggerSelector); // Устанавливаем обработчики на кнопки вызова модального окна
...
function slider({
  ...
  // Элемент слайдера
  const sliderElem = document.querySelector(container); // Стрелка "назад"

  const sliderCounterPrevElem = sliderElem ? sliderElem.querySelector(prevArrow) : null; // Стрелка "вперёд"

  const sliderCounterNextElem = sliderElem ? sliderElem.querySelector(nextArrow) : null; // Элемент с номером текущего слайда

  const sliderCounterCurrentElem = sliderElem ? sliderElem.querySelector(currentCounter) : null; // Элемент с номером общего количества слайдов

  const sliderCounterTotalElem = sliderElem ? sliderElem.querySelector(totalCounter) : null; // Контейнер слайдов

  const sliderWrapperElem = sliderElem ? sliderElem.querySelector(wrapper) : null; // "Карусель слайдов"

  const sliderCarouselElem = sliderWrapperElem ? sliderWrapperElem.querySelector(carousel) : null; // Элементы слайдов

  const slideElems = sliderWrapperElem ? sliderWrapperElem.querySelectorAll(slide) : null; // Индекс стартового слайда

  const START_SLIDE_INDEX = 0; // Ширина слайдера

  const sliderWidth = window.getComputedStyle(sliderWrapperElem).width; // Создаёт элемент навигации

  const createNavigationElem = () => {
    // Создаём сам элемент навигации
    const navigationElem = document.createElement('ol');
    navigationElem.classList.add('offer__slider-carousel-indicators'); // Создаём фрагмент с индикаторами слайдов

    const slideIndicatorsFrag = document.createDocumentFragment(); // Заполняем фрагмент
  ...
  sliderElem.append(sliderNavigationElem); // Устанавливает ширину "карусели"
  ...
  const switchToSlide = index => {
    ...
    deactivateAllSlides();
    activateSlide(index); // Подсвечиваем индикатор слайда
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, tabsActiveClass, tabsContentHideClass) {
  ...
  tabheaderItemsParentElem = document.querySelector(tabsParentSelector); // Скрывает все табы
...
function timer(timerSelector, deadline) {
  ...
  const setClock = (selector, endtime) => {
    ...
    const updateClock = () => {
      ...
      seconds.innerHTML = Object(_services_services__WEBPACK_IMPORTED_MODULE_0__["setZero"])(time.seconds); // Очищаем интервал, если целевое время достигнуто
